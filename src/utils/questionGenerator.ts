...
function findSimilarCharacters(targetCharacter: Character, allCharacters: Character[], count: number): Character[] {
...
export function generateFlashcardQuestion(
  ...
  const similarCharacters = findSimilarCharacters(targetCharacter, allCharacters, 3);
  ...
  while (options.length < 4) {
    ...
    if (!usedAbilities.has(modifiedAbility)) {
      ...
      const additionalCharacters = findSimilarCharacters(targetCharacter, allCharacters, 10);
      let added = false;
...
export function generateQuestionsFromCharacters(characters: Character[]): FlashcardQuestion[] {
  return characters.map(character => generateFlashcardQuestion(character, characters));
}

// Function to select next character based on progress (prioritize incorrect answers)
export function selectNextCharacter(
  characters: Character[],
  progressMap: Map<string, { correct: number; incorrect: number; lastSeen: Date }>
): Character {
  ...
  const characterScores = characters.map(character => {
    ...
    const totalScore = incorrectScore + correctScore + timeScore + accuracyScore;
  ...
  const topCharacters = characterScores.slice(0, Math.min(3, characterScores.length));
